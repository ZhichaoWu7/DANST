...
def draw_pie(dist, xpos, ypos, size, colors, ax):
    ...
    for r1, r2 in zip(pie[:-1], pie[1:]):
        ...
        if i < len(colors):
            ...
            ax.scatter([xpos], [ypos], marker=xy, s=size, c='gray', edgecolors='none')  
        i += 1
...
def plot_frac_results(predict, cell_type_list, coordinates, file_name=None, point_size=1000, size_coefficient=0.0009, if_show=True, color_dict=None):
    ...
    if color_dict != None:
        ...
        if len(labels) <= 10:
            ...
            import matplotlib
            color = plt.get_cmap('viridis', len(labels))
            colors = []
    ...
    for i in range(predict.shape[0]):
        ax = draw_pie(predict.iloc[i], coordinates['coor_X'].values[i], coordinates['coor_Y'].values[i], 
...
def plot_scatter_by_type(predict, cell_type_list, coordinates, point_size=400, size_coefficient=0.0009, file_path=None,
    ...
    for i in range(len(cell_type_list)):
        ...
        cm = plt.cm.get_cmap('Reds')
        ax = plt.scatter(coordinates['coor_X'], coordinates['coor_Y'], s=point_size, vmin=0, vmax=1, c=predict.iloc[:, i],
                         cmap=cm)

        ax.set_title(f"Cell Type: {cell_type_list[i]}")
        plt.colorbar(ax, label='Prediction Value')  
        plt.show() 
